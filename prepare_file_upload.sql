...
SET @intangible_asset = CONCAT('SELECT `asset_number`, `category`, DATE_FORMAT(dpis, "%m/%d/%Y"), `cost`,`dm',@month,'_',@year,'`,`dm',@month,'_',@year,'`+`akum_upto_prev_Year`, "FISCAL", 1, source_detail, @addition_date
			FROM `intangible_asset_',@filedate,'`');
...
UNION ALL		
SELECT `asset_number_wo`, `category`, DATE_FORMAT(dpis, "%m/%d/%Y"), 0 , 0, `charging_depre`, "FISCAL", 1 ,"Manual 2017", @addition_date FROM cit_asset.`write_off_depre`
;');
PREPARE stmt FROM @stmt;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
