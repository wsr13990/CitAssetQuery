...
USE `cit_asset`$$
DROP PROCEDURE IF EXISTS `get_pivot_by_month_year_source_table`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_pivot_by_month_year_source_table`(paramMonth INTEGER(2), paramYear INTEGER(4), paramSource VARCHAR(25), IN tableName VARCHAR(25))
READS SQL DATA
BEGIN
